...
async function f() {
	...
	const friendIdJson = await myFetch(`friendsOf${user.id}.json`);

	friendIdJson.friendIds.forEach(async id => {
		const friend = await myFetch(`user${id}.json`);
		const post = await myFetch(`message${friend.latestMsgId}.json`);
		console.log(`${friend.name} says: ${post.message}`);
	})
}
