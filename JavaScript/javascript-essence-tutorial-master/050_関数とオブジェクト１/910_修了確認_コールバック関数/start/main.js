...
function calcFactory(val) {
    return {
        plus: function(target) {
            ...
            const returnValue = `${val} + ${target} = ${newVal}`;
            val = newVal;
            return returnValue;
        },
        ...
        multiply: function(target) {
            ...
            const returnValue = `${val} x ${target} = ${newVal}`;
            val = newVal;
            return returnValue;
        },
...
const outputConsole = function(cb){
    console.log(cb);
}

const outputAlert = function(cb){
    alert(cb);
}

outputAlert(calc.plus(5));
outputAlert(calc.minus(3));
outputAlert(calc.multiply(3));
outputAlert(calc.divide(2));
